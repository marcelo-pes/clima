...
type EcowittPeriod = "24h" | "weekly" | "monthly" | "yearly" | "7d";
...
const RANGE_LABELS: Record<RangeKey, string> = { "24h": "24 horas", "7d": "Semanal", "30d": "Mensal", "1y": "Anual" };
...
function chartDomain(history: Record<string, Series>, lines: { key: string }[], unit: string): [number, number] {
  const values = lines.filter((line) => (history[line.key]?.unit || line.key) === unit)
    .flatMap((line) => history[line.key]?.points.map((point) => point.value) ?? [])
    .filter(Number.isFinite);
  return values.length ? paddedChartDomain([Math.min(...values), Math.max(...values)]) : [0, 1];
}
